...
def get_oasis(history_map: list[list[int]]) -> int:
    ...
    for history in history_map:
        ...
        counter += calc_history_value(pyramid)
    return counter
...
def calc_history_value(pyramid: list[list[int]]) -> int:
    pyramid.reverse()
    final_history_value = pyramid[0][-1] + pyramid[1][-1]
    for idx in range(1, len(pyramid)-1):
        final_history_value = final_history_value + pyramid[idx+1][-1]
